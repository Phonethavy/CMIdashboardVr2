    ...
    <div class="container mt-4">
        ...
        <table class="table1" id="data-table">
            ...
            <tbody>
                <tr id="calculated-ratio-row">
                    <td style="padding: 5px 0px;text-align: left;" data-label="Data Type">ອັດຕາສ່ວນຄວາມພຽງພໍຂອງການຫັກເງິນແຮສິນເຊື່ອທີ່ຖືກຈັດຊັ້ນ / ເງິນແຮທັງໝົດທີ່ຕ້ອງສ້າງຕາມລະບຽບ ຕ້ອງຫຼາຍກວ່າ</td>
        ...
        <script>
            function calculate21(provisionA, provisionB, provisionC, provisionD, provisionE) {
                ...
                return denominator ? (((provisionA * (1 / 100)) + (provisionB * (5 / 100)) + (provisionC * (25 / 100)) + (provisionD * (50 / 100) + (provisionE * (100 / 100))))) / denominator * 100 .toFixed(2) : "";
            }
            ...
            function createChart(labels, data1, data2) {
                ...
                financialChart = new Chart(ctx, {
                    ...
                    data: {
                        labels: labels,
                        datasets: [{
                                label: 'ອັດຕາສ່ວນສິນເຊື່ອໃຫ້ແກ່ພາກສ່ວນທີ່ມີສາຍພົວພັນທັງໝົດ / ທຶນທັງໝົດ ຕ້ອງນ້ອຍກວ່າ',
                                data: data2,
                ...
                $('#fetch-data').on('click', function() {
                    ...
                    Promise.all(promises).then(() => {
                        ...
                        Object.keys(fetchedData).forEach(month => {
                            ...
                            const roe = calculate21(provisionA, provisionB, provisionC, provisionD, provisionE);
                            roeData.push(roe);
                            labels.push(month);
                    ...
                    if (selectedYear) {
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                var rows = {
                                    provisionA: `<tr><td>ຫັກແຮຊັ້ນ A</td>`,
                                    provisionB: `<tr><td>ຫັກແຮຊັ້ນ B</td>`,
                                    provisionC: `<tr><td>ຫັກແຮຊັ້ນ C</td>`,
                                    provisionD: `<tr><td>ຫັກແຮຊັ້ນ D</td>`,
                                    provisionE: `<tr><td>ຫັກແຮຊັ້ນ E</td>`,
                                ...
                                for (var month in data) {
                                    ...
                                    var ratio = calculate21(data[month].provisionA, data[month].provisionB, data[month].provisionC, data[month].provisionD, data[month].provisionE);
                                    rows.provisionA += `<td>${numberFormat(data[month]. provisionA*(1/100))}</td>`;
                                    rows.provisionB += `<td>${numberFormat(data[month].provisionB*(5/100))}</td>`;
                                    rows.provisionC += `<td>${numberFormat(data[month]. provisionC*(25/100))}</td>`;
                                    rows.provisionD += `<td>${numberFormat(data[month]. provisionD*(50/100))}</td>`;
                                    rows.provisionE += `<td>${numberFormat(data[month]. provisionE*(100/100))}</td>`;
