                ...
                $('#fetch-data').on('click', function() {
                    ...
                    Promise.all(promises).then(() => {
                        ...
                        Object.keys(fetchedData).forEach(month => {
                            ...
                            const roe = calculate3(result_monthV, CapitalPreviousYear, Capital16, debt2);
                            roeData.push(roe);
                            labels.push(month);
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                var rows = {
                                    ...
                                    CapitalPreviousYear: `<tr><td>ທືນທັງໝົດປີກ່ອນໜ້າ  (${previousYear})</td>`,
                                ...
                                for (var month in data) {
                                    ...
                                    var ratio = calculate3(data[month].result_monthV, data[month].CapitalPreviousYear, data[month].Capital16);

                                    rows.result_monthV += `<td>${numberFormat(data[month].result_monthV)}</td>`;
                                    rows.CapitalPreviousYear += `<td>${numberFormat(data[month].CapitalPreviousYear)}</td>`;
