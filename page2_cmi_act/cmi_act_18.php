        ...
        <table class="table1" id="data-table">
            ...
            <tbody>
                <tr id="calculated-ratio-row">
                    <td style="padding: 5px 0px;text-align: left;" data-label="Data Type">ອັດຕາສ່ວນສິນເຊື່ອໃຫ້ແກ່ລູກຄ້າໜຶ່ງລາຍ / ທຶນທັງໝົດ ຕ້ອງນ້ອຍກວ່າ</td>
            ...
            function createChart(labels, data1, data2) {
                ...
                financialChart = new Chart(ctx, {
                    ...
                    data: {
                        labels: labels,
                        datasets: [{
                                label: 'ອັດຕາສ່ວນສິນເຊື່ອໃຫ້ແກ່ລູກຄ້າລາຍໃຫຍ່ / ທຶນທັງໝົດ ຕ້ອງນ້ອຍກວ່າ',
                                data: data2,
                ...
                $('#fetch-data').on('click', function() {
                    ...
                    Promise.all(promises).then(() => {
                        ...
                        Object.keys(fetchedData).forEach(month => {
                            ...
                            const roe = calculate18(Capital16, outstanding05, Quantity);
                            roeData.push(roe);
                            labels.push(month);
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                var rows = {
                                    ...
                                    outstanding05: `<tr><td>Outstanding (ຍອດເຫຼືອເງິນກູ້ທັງໝົດ)</td>`,
                                    Quantity: `<tr><td>Quantity (ຍອດເຫຼືອລູກຄ້າ)</td>`,
                                ...
                                for (var month in data) {
                                    ...
                                    var ratio = calculate18(data[month].outstanding05, data[month].outstanding05, data[month].Quantity);

                                    rows.Capital16 += `<td>${numberFormat(data[month].Capital16)}</td>`;
                                    rows.outstanding05 += `<td>${numberFormat(data[month].outstanding05)}</td>`;
                                    rows.Quantity += `<td>${numberFormat(data[month].Quantity)}</td>`;
