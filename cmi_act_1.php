                ...
                $('#fetch-data').on('click', function() {
                    ...
                    Promise.all(promises).then(() => {
                        ...
                        Object.keys(fetchedData).forEach(month => {
                            ...
                            const roe = calculate(Capital16, debt2, loans5, assets8, otherAssets10);
                            roeData.push(roe);
                            labels.push(month);
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                for (var month in data) {
                                    ...
                                    var ratio = calculate(data[month].Capital16, data[month].debt2, data[month].assets8, data[month].loans5, data[month].otherAssets10);

                                    rows.Capital16 += `<td>${numberFormat(data[month].Capital16)}</td>`;
                                    rows.debt2 += `<td>${numberFormat(data[month].debt2)}</td>`;
                                    rows.assets8 += `<td>${numberFormat(data[month].assets8)}</td>`;
                                    rows.loans5 += `<td>${numberFormat(data[month].loans5)}</td>`;
                                    rows.otherAssets10 += `<td>${numberFormat(data[month].otherAssets10)}</td>`;
