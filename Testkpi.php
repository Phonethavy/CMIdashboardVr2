...
FROM `tbl_loan` WHERE `loan_receive_finish_date` BETWEEN ? AND ? "; //ลูกค้าเสนอกู้
    ...
    $previous_month_end = date('Y-m-t', strtotime('-1 month', strtotime($start_date)));
    // ตัวอย่าง $previous_month_end  คือ '2024-11-30'  (ถ้า $start_date คือ 2024-12-14)
    // สร้างคำสั่ง SQL ใหม่
    $sql_tier = "SELECT 
    SUM(CASE WHEN tier.cus_prov_id_fk = 16 AND tier.payment_status = 'A' AND tier.payment_status = 'B' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_16A_B,
    SUM(CASE WHEN tier.cus_prov_id_fk = 15 AND tier.payment_status = 'A' AND tier.payment_status = 'B' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_15A_B,
    SUM(CASE WHEN tier.cus_prov_id_fk = 14 AND tier.payment_status = 'A' AND tier.payment_status = 'B' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_14A_B,
    SUM(CASE WHEN tier.cus_prov_id_fk = 17 AND tier.payment_status = 'A' AND tier.payment_status = 'B' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_17A_B,

    SUM(CASE WHEN tier.cus_prov_id_fk = 14 AND tier.payment_status = 'C' AND tier.payment_status = 'D' AND tier.payment_status = 'E' AND tier.payment_status = 'write-off' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_par6014,
    SUM(CASE WHEN tier.cus_prov_id_fk = 15 AND tier.payment_status = 'C' AND tier.payment_status = 'D' AND tier.payment_status = 'E' AND tier.payment_status = 'write-off' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_par6015,
    SUM(CASE WHEN tier.cus_prov_id_fk = 16 AND tier.payment_status = 'C' AND tier.payment_status = 'D' AND tier.payment_status = 'E' AND tier.payment_status = 'write-off' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_par6016,
    SUM(CASE WHEN tier.cus_prov_id_fk = 17 AND tier.payment_status = 'C' AND tier.payment_status = 'D' AND tier.payment_status = 'E' AND tier.payment_status = 'write-off' THEN pay.pay_amnt ELSE 0 END) AS total_pay_amnt_par6017,
