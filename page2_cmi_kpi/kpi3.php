...
<body>
    ...
    <div class="popup" id="popup">
        ...
        <script>
            ...
            function createChart(labels, data1, data2, goalValue) {
                if (financialChart) {
                ...
                let minValue = Math.min(...sanitizedData, ...sanitizedGoal) * 0.8;

                // 🔎 Debug: ตรวจสอบค่าหลังแปลง
                console.log("Sanitized KPI3:", sanitizedData);
                console.log("Sanitized Goal:", sanitizedGoal);

                // ❌ ป้องกันกราฟไม่ขึ้น
                if (sanitizedData.every(v => v === 0) && sanitizedGoal.every(v => v === 0)) {
                    console.warn("⚠ ไม่มีข้อมูลที่ใช้สร้างกราฟ");
                    ctx.fillStyle = "gray";
                    ctx.font = "16px Arial";
                    ctx.fillText("ไม่มีข้อมูล", 50, 50);
                    return;
                }
            ...
            document.getElementById('searchForm').addEventListener('submit', function(event) {
                ...
                fetch(`../ajx_json/kpi3_db.php?year=${year}`)
                    ...
                    .then(data => {
                        ...
                        let categories = ['KPI3'];
                        let goalValue = (year == "2024") ? 26000 : 28000;
                        let firstRow = true;
